...
const BASE_URL = process.env.SCRAPE_BASE_URL;
if (!BASE_URL) {
  throw new Error("Falta la variable de entorno SCRAPE_BASE_URL");
}

export async function scrapeSingleProduct(slug) {
  const url = `${BASE_URL}${slug.trim()}`;
  ...
  throw createError(404, "Producto no encontrado en pccomponentes");
}
